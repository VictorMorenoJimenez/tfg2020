...
@Injectable()
export class TripsReadFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  constructor(
    ...
    this.cacheSub = {};
    this.cache = 'TripsRead';
  }

  submit(value: any = false, cache: boolean = true, only_cache: boolean = false): Observable<__model.Trip> {
    if (value === false) {
  ...
  try(subject: ReplaySubject<__model.Trip>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    ...
    const result = this.tripsService.read(value);
    result.pipe(
      map(val => {
