...
@Injectable()
export class TripsPartialUpdateFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  constructor(
    ...
    this.form = new FormGroup({
      data: new FormGroup({
        ...
        departure_date: new FormControl({value: undefined, disabled: false}, [Validators.required]),
        return_date: new FormControl({value: undefined, disabled: false}, [Validators.required]),
    ...
    this.cacheSub = {};
    this.cache = 'TripsPartialUpdate';
  }

  submit(value: any = false): Observable<__model.Trip> {
    const cache = false;
    ...
    this.cacheSub[JSON.stringify(value) + cache] = new ReplaySubject<__model.Trip>(1);
  ...
  try(subject: ReplaySubject<__model.Trip>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    const result = this.tripsService.partialUpdate(value);
    result.pipe(
      map(val => {
