...
@Injectable()
export class PydrfpermissionsPydrfpermissionsFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  constructor(
    private pydrfpermissionsService: PydrfpermissionsService,
    ...
    this.cacheSub = {};
    this.cache = 'PydrfpermissionsPydrfpermissions';
  }

  submit(value: any = false, cache: boolean = true, only_cache: boolean = false): Observable<__model.PermissionDRF> {
    if (value === false) {
      value = this.form.value;
    }
    this.cacheSub[JSON.stringify(value) + cache] = new ReplaySubject<__model.PermissionDRF>(1);
  ...
  try(subject: ReplaySubject<__model.PermissionDRF>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    if (JSON.stringify(value) !== JSON.stringify(this.currentValue)) {
      subject.complete();
      delete this.cacheSub[JSON.stringify(value) + cache];
      return;
    }
    const result = this.pydrfpermissionsService.pydrfpermissions(value);
    result.pipe(
      map(val => {
