...
@Injectable()
export class AuthLoginFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  constructor(
    ...
    this.form = new FormGroup({
      data: new FormGroup({
        ...
        email: new FormControl({value: undefined, disabled: false}, [Validators.email]),
    ...
    this.cacheSub = {};
    this.cache = 'AuthLogin';
  }

  submit(value: any = false): Observable<__model.Login> {
    const cache = false;
    const only_cache = false;
    if (value === false) {
      value = this.form.value;
    }
    this.cacheSub[JSON.stringify(value) + cache] = new ReplaySubject<__model.Login>(1);
  ...
  try(subject: ReplaySubject<__model.Login>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    const result = this.authService.login(value);
    result.pipe(
      map(val => {
