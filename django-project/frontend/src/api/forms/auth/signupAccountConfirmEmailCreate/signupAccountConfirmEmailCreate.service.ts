...
@Injectable()
export class AuthSignupAccountConfirmEmailCreateFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  constructor(
    ...
    this.cacheSub = {};
    this.cache = 'AuthSignupAccountConfirmEmailCreate';
  }
  ...
  try(subject: ReplaySubject<string>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    const result = this.authService.signupAccountConfirmEmailCreate(value);
    result.pipe(
      map(val => {
