...
@Injectable()
export class AuthConfirmFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  constructor(
    ...
    this.form = new FormGroup({
      data: new FormGroup({
        ...
        uid: new FormControl({value: undefined, disabled: false}, [Validators.minLength(1), Validators.required]),
    ...
    this.cacheSub = {};
    this.cache = 'AuthConfirm';
  }

  submit(value: any = false): Observable<__model.PasswordResetConfirm> {
    const cache = false;
    const only_cache = false;
    if (value === false) {
      value = this.form.value;
    }
    this.cacheSub[JSON.stringify(value) + cache] = new ReplaySubject<__model.PasswordResetConfirm>(1);
  ...
  try(subject: ReplaySubject<__model.PasswordResetConfirm>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    const result = this.authService.confirm(value);
    result.pipe(
      map(val => {
