...
@Injectable()
export class AuthResetFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  constructor(
    ...
    this.cacheSub = {};
    this.cache = 'AuthReset';
  }

  submit(value: any = false): Observable<__model.CustomPasswordReset> {
    const cache = false;
    const only_cache = false;
    if (value === false) {
      value = this.form.value;
    }
    this.cacheSub[JSON.stringify(value) + cache] = new ReplaySubject<__model.CustomPasswordReset>(1);
  ...
  try(subject: ReplaySubject<__model.CustomPasswordReset>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    const result = this.authService.reset(value);
    result.pipe(
      map(val => {
