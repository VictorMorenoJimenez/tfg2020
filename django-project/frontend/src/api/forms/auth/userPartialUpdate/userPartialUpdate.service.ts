...
@Injectable()
export class AuthUserPartialUpdateFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  constructor(
    ...
    this.form = new FormGroup({
      data: new FormGroup({
        ...
        password: new FormControl({value: undefined, disabled: false}, [Validators.maxLength(128), Validators.minLength(1), Validators.required]),
    ...
    this.cacheSub = {};
    this.cache = 'AuthUserPartialUpdate';
  }
  ...
  try(subject: ReplaySubject<__model.User>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    const result = this.authService.userPartialUpdate(value);
    result.pipe(
      map(val => {
