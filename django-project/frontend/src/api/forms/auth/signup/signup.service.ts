...
export class AuthSignupFormService {
  ...
  constructor(
    ...
    this.form = new FormGroup({
      data: new FormGroup({
        ...
        password1: new FormControl({value: undefined, disabled: false}, [Validators.minLength(1), Validators.required]),
        password2: new FormControl({value: undefined, disabled: false}, [Validators.minLength(1), Validators.required]),
      }, [Validators.required]),
    ...
    this.cacheSub = {};
    this.cache = 'AuthSignup';
  }

  submit(value: any = false): Observable<__model.Register> {
    const cache = false;
    const only_cache = false;
    if (value === false) {
      value = this.form.value;
    }
    this.cacheSub[JSON.stringify(value) + cache] = new ReplaySubject<__model.Register>(1);
  ...
  try(subject: ReplaySubject<__model.Register>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    const result = this.authService.signup(value);
    result.pipe(
      map(val => {
