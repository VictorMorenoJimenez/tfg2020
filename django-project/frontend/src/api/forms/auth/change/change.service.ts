...
@Injectable()
export class AuthChangeFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  constructor(
    ...
    this.form = new FormGroup({
      data: new FormGroup({
        old_password: new FormControl({value: undefined, disabled: false}, [Validators.maxLength(128), Validators.minLength(1), Validators.required]),
        new_password1: new FormControl({value: undefined, disabled: false}, [Validators.maxLength(128), Validators.minLength(1), Validators.required]),
        new_password2: new FormControl({value: undefined, disabled: false}, [Validators.maxLength(128), Validators.minLength(1), Validators.required]),
      }, [Validators.required]),
    ...
    this.cacheSub = {};
    this.cache = 'AuthChange';
  }

  submit(value: any = false): Observable<__model.PasswordChange> {
    const cache = false;
    const only_cache = false;
    if (value === false) {
      value = this.form.value;
    }
    this.cacheSub[JSON.stringify(value) + cache] = new ReplaySubject<__model.PasswordChange>(1);
  ...
  try(subject: ReplaySubject<__model.PasswordChange>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    const result = this.authService.change(value);
    result.pipe(
      map(val => {
