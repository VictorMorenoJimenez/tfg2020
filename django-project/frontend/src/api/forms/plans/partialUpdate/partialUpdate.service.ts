...
@Injectable()
export class PlansPartialUpdateFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  constructor(
    ...
    this.cacheSub = {};
    this.cache = 'PlansPartialUpdate';
  }

  submit(value: any = false): Observable<__model.Plan> {
    const cache = false;
    ...
    this.cacheSub[JSON.stringify(value) + cache] = new ReplaySubject<__model.Plan>(1);
  ...
  try(subject: ReplaySubject<__model.Plan>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    const result = this.plansService.partialUpdate(value);
    result.pipe(
      map(val => {
