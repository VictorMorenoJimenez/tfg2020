...
@Injectable()
export class PlansReadFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  constructor(
    ...
    this.cacheSub = {};
    this.cache = 'PlansRead';
  }

  submit(value: any = false, cache: boolean = true, only_cache: boolean = false): Observable<__model.Plan> {
    if (value === false) {
  ...
  try(subject: ReplaySubject<__model.Plan>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    ...
    const result = this.plansService.read(value);
    result.pipe(
      map(val => {
