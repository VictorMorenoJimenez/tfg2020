...
@Injectable()
export class TravelersCreateFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  constructor(
    ...
    this.form = new FormGroup({
      data: new FormGroup({
        ...
        username: new FormControl({value: undefined, disabled: false}, [Validators.maxLength(150), Validators.minLength(1), Validators.pattern(/^[\w.@+-]+$/), Validators.required]),
    ...
    this.cacheSub = {};
    this.cache = 'TravelersCreate';
  }

  submit(value: any = false): Observable<__model.TravelerCreate> {
    const cache = false;
    const only_cache = false;
    if (value === false) {
      value = this.form.value;
    }
    this.cacheSub[JSON.stringify(value) + cache] = new ReplaySubject<__model.TravelerCreate>(1);
  ...
  try(subject: ReplaySubject<__model.TravelerCreate>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    const result = this.travelersService.create(value);
    result.pipe(
      map(val => {
