...
@Injectable()
export class DestinationsReadFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  constructor(
    ...
    this.cacheSub = {};
    this.cache = 'DestinationsRead';
  }

  submit(value: any = false, cache: boolean = true, only_cache: boolean = false): Observable<__model.Destination> {
    if (value === false) {
  ...
  try(subject: ReplaySubject<__model.Destination>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    ...
    const result = this.destinationsService.read(value);
    result.pipe(
      map(val => {
