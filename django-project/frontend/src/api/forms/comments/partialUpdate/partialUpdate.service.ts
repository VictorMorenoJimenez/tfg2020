...
@Injectable()
export class CommentsPartialUpdateFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  constructor(
    ...
    this.cacheSub = {};
    this.cache = 'CommentsPartialUpdate';
  }

  submit(value: any = false): Observable<__model.Comment> {
    const cache = false;
    ...
    this.cacheSub[JSON.stringify(value) + cache] = new ReplaySubject<__model.Comment>(1);
  ...
  try(subject: ReplaySubject<__model.Comment>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    const result = this.commentsService.partialUpdate(value);
    result.pipe(
      map(val => {
