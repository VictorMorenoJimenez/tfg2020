...
@Injectable()
export class CommentsRepliesReadFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  constructor(
    ...
    this.cacheSub = {};
    this.cache = 'CommentsRepliesRead';
  }

  submit(value: any = false, cache: boolean = true, only_cache: boolean = false): Observable<__model.Reply> {
    if (value === false) {
  ...
  try(subject: ReplaySubject<__model.Reply>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    ...
    const result = this.commentsService.repliesRead(value);
    result.pipe(
      map(val => {
