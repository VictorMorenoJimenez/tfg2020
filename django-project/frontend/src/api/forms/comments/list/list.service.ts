...
export class CommentsListFormService {
  ...
  constructor(
    ...
    this.form = new FormGroup({
      ...
      plan: new FormControl({value: undefined, disabled: false}, []),
    ...
    this.cacheSub = {};
    this.cache = 'CommentsList';
  }

  submit(value: any = false, cache: boolean = true, only_cache: boolean = false): Observable<__model.CommentsList> {
    if (value === false) {
      value = this.form.value;
    }
    this.cacheSub[JSON.stringify(value) + cache] = new ReplaySubject<__model.CommentsList>(1);
  ...
  try(subject: ReplaySubject<__model.CommentsList>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    if (JSON.stringify(value) !== JSON.stringify(this.currentValue)) {
      subject.complete();
      delete this.cacheSub[JSON.stringify(value) + cache];
      return;
    }
    const result = this.commentsService.list(value);
    result.pipe(
      map(val => {
