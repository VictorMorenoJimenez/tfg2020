...
@Injectable()
export class CommentsReadFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  constructor(
    ...
    this.cacheSub = {};
    this.cache = 'CommentsRead';
  }

  submit(value: any = false, cache: boolean = true, only_cache: boolean = false): Observable<__model.Comment> {
    if (value === false) {
  ...
  try(subject: ReplaySubject<__model.Comment>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    ...
    const result = this.commentsService.read(value);
    result.pipe(
      map(val => {
