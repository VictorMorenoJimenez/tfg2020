...
@Injectable()
export class CorePermissionReadFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  constructor(
    ...
    this.cacheSub = {};
    this.cache = 'CorePermissionRead';
  }

  submit(value: any = false, cache: boolean = true, only_cache: boolean = false): Observable<__model.Permission> {
    if (value === false) {
      value = this.form.value;
    }
    this.cacheSub[JSON.stringify(value) + cache] = new ReplaySubject<__model.Permission>(1);
  ...
  try(subject: ReplaySubject<__model.Permission>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    if (JSON.stringify(value) !== JSON.stringify(this.currentValue)) {
      subject.complete();
      delete this.cacheSub[JSON.stringify(value) + cache];
      return;
    }
    const result = this.coreService.permissionRead(value);
    result.pipe(
      map(val => {
