...
@Injectable()
export class CoreContentTypeListFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  constructor(
    ...
    this.cacheSub = {};
    this.cache = 'CoreContentTypeList';
  }

  submit(value: any = false, cache: boolean = true, only_cache: boolean = false): Observable<__model.CoreContentTypeList> {
    if (value === false) {
      value = this.form.value;
    }
    this.cacheSub[JSON.stringify(value) + cache] = new ReplaySubject<__model.CoreContentTypeList>(1);
  ...
  try(subject: ReplaySubject<__model.CoreContentTypeList>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    if (JSON.stringify(value) !== JSON.stringify(this.currentValue)) {
      subject.complete();
      delete this.cacheSub[JSON.stringify(value) + cache];
      return;
    }
    const result = this.coreService.contentTypeList(value);
    result.pipe(
      map(val => {
