...
@Injectable()
export class CoreRegisterFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  constructor(
    ...
    this.cacheSub = {};
    this.cache = 'CoreRegister';
  }

  submit(value: any = false): Observable<__model.Phone> {
    const cache = false;
    const only_cache = false;
    if (value === false) {
      value = this.form.value;
    }
    this.cacheSub[JSON.stringify(value) + cache] = new ReplaySubject<__model.Phone>(1);
  ...
  try(subject: ReplaySubject<__model.Phone>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    const result = this.coreService.register(value);
    result.pipe(
      map(val => {
