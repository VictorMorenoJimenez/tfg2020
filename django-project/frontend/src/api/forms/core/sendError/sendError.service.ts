...
@Injectable()
export class CoreSendErrorFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  constructor(
    ...
    this.cacheSub = {};
    this.cache = 'CoreSendError';
  }

  submit(value: any = false, cache: boolean = true, only_cache: boolean = false): Observable<__model.CoreSendError> {
    if (value === false) {
      value = this.form.value;
    }
    this.cacheSub[JSON.stringify(value) + cache] = new ReplaySubject<__model.CoreSendError>(1);
  ...
  try(subject: ReplaySubject<__model.CoreSendError>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    if (JSON.stringify(value) !== JSON.stringify(this.currentValue)) {
      subject.complete();
      delete this.cacheSub[JSON.stringify(value) + cache];
      return;
    }
    const result = this.coreService.sendError(value);
    result.pipe(
      map(val => {
