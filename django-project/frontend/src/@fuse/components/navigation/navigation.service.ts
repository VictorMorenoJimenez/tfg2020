...
import {DashboardDashboardFormService} from '../../../api/forms/dashboard/dashboard/dashboard.service';
...
export class FuseNavigationService
{
     ...
     * Constructor
     */
    constructor(private notificationsFS: DashboardDashboardFormService)
    {
        // Set the defaults
     ...
     * @param navigation
     */
    updateNavigationItem(id, properties, navigation = null): void
    {
    ...
    reloadPermissions(isAllowed, parent): void{
        // let that=this;
        const nav = parent.navigation;
        this.reloadPermissionsNav(nav, isAllowed, parent);
    }

    reloadPermissionsNav(nav, isAllowed, parent): void{
        for (const i in nav){
            if (nav[i].children){
                this.reloadPermissionsNav(nav[i].children, isAllowed, parent);
                this.updateNavigationItem(nav[i].id, {
                    hidden: !this.hasChildrenShow(nav[i].children)
                }, nav);
            }else{
                this.updateNavigationItem(nav[i].id, {
                    hidden: !isAllowed(nav[i], parent)
                }, nav);
            }
        }
    }

    reloadBadges(navigation?): void{
        const nav = navigation || this.getCurrentNavigation();
        this.notificationsFS.submit(false, true, true).pipe(
            map(data => {
                for (const i in nav){
                    if (nav[i].children){
                        this.reloadBadges(nav[i].children);
                    }
                    for (const d in data){
                        if (data[d].model === nav[i].id){
                            if (nav[i].badge){
                                nav[i].badge.title = data[d].count;
                                this.updateNavigationItem(nav[i].id, nav[i]);
                            }
                        }
                    }
                }
    ...
    hasChildrenShow(children): boolean{
